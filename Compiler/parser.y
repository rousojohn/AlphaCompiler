...
expr:	assignexpr {  $$ = $1; if(printRules)  printf(" expr: assignexpr\n");  }
	       ...
	       | expr BIGGER expr 
			{
				...
				push($$->trueList, nextquadlabel() );
				push($$->falseList, nextquadlabel() +1);
	       ...
	       | expr BIGGEREQUAL expr
			{ 
				...
				push($$->trueList, nextquadlabel() );
				push($$->falseList, nextquadlabel() +1);
				emit(if_greateq, $1, $3, NULL, 0, yylineno); 
				emit(jump,NULL,NULL,NULL,0,yylineno);
	       ...
	       | expr AND M expr
			{ 
				...
				backpatch($$->trueList,nextquadlabel() );
				backpatch($$->falseList, nextquadlabel()+2);	
 
				emit(assign,$$,NULL,getExprConstBool("true"),0,yylineno);
				emit(jump,NULL,NULL,NULL,nextquadlabel()+1,yylineno);
				emit(assign,$$,NULL,getExprConstBool("false"),0,yylineno);
	      ...
	      | term {  $$ = $1; /**/if(printRules)  printf(" expr: term\n"); }
	       ;
...
assignexpr:	lvalue
			{
				...
				/*	emit(assign, $4, (expr1*)0, $1, 0, yylineno);
					emit(assign, getExprConstBool("true"), (expr1*)0, $1, 0, yylineno);
...
forprefix:	FOR LEFT_PARENTHESIS elist QUESTIONMARK M expr QUESTIONMARK
					{	
						...
						emit(if_eq, $6, getExprConstBool("true"), NULL, 0, yylineno);	
					}
				;
